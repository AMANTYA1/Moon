...
async def channel_toggle(db, message: Message):
    ...
    if status == "on":
        ...
        if chat_id in db:
            db.remove(chat_id)
            return await tbot(message, text="antichannel Disabled!")
        await tbot(message, text=f"**Anti Channel Mode Successfully Deactivated In The Chat** {message.chat.id} ❌")
    else:
        await tbot(message, text="I undestand `/antichannel on` and `/antichannel off` only")
...
async def antichannel_status(_, message: Message):
    if len(message.command) != 2:
        return await tbot(message, text="I undestand `/antichannel on` and `/antichannel off` only")
